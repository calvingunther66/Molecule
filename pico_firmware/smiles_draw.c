...
void parse_smiles_to_graph(const char* smiles, MolGraph* graph) {
    ...
    for (size_t i = 0; i < len; ++i) {
        ...
        if (isalpha(c)) {
            // Found an atom
            if (graph->num_nodes >= MAX_DRAW_NODES) break;
            
            // To handle two-letter atoms like 'Cl', 'Br', just take first letter for simplicity right now
            if (i+1 < len && islower(smiles[i+1]) && c != 'c' && c != 'n' && c != 'o') {
        ...
        // Ignore -, =, #, for basic topology we just need the edges regardless of bond order.
    }
}
...
void calculate_2d_layout(MolGraph* graph) {
    ...
    for (int iter = 0; iter < MAX_ITERATIONS; ++iter) {
        ...
        for (int v = 0; v < graph->num_nodes; ++v) {
            ...
            if (graph->x[v] < 5.0f) graph->x[v] = 5.0f;
            if (graph->x[v] > AREA_WIDTH - 5.0f) graph->x[v] = AREA_WIDTH - 5.0f;
            if (graph->y[v] < 5.0f) graph->y[v] = 5.0f;
            if (graph->y[v] > AREA_HEIGHT - 5.0f) graph->y[v] = AREA_HEIGHT - 5.0f;
        }
    ...
    for (int v = 0; v < graph->num_nodes; ++v) {
        if(graph->x[v] < min_x) min_x = graph->x[v];
        if(graph->x[v] > max_x) max_x = graph->x[v];
        if(graph->y[v] < min_y) min_y = graph->y[v];
        if(graph->y[v] > max_y) max_y = graph->y[v];
    }
